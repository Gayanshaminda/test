...
const Governancevotes: React.FC<GovernancevotesProps> = ({ onClick, description }) => {
  ...
  return (
    <div className="flex w-full relative pointer-events-auto">
      {(!isLargeScreen && !isExpanded) ? (
        <button 
          ...
          `}
        >
          <span className="text-base sm:text-lg font-[900] font-['Product_Sans_Black']">Governance Votes</span>
          <img 
            src={parkdownIcon} 
            alt="Expand" 
        ...
        >
          <div className={`flex items-center ${isLargeScreen ? 'justify-center' : 'justify-between'} mb-3 sm:mb-3 md:mb-4 lg:mb-3 xl:mb-3`}>
            <h3 className={`
              ...
              <span className="text-accent-pink">2 : </span>Governance Votes
            </h3>
            {!isLargeScreen && (
              <img 
                src={parkdownIcon} 
            ...
            `}>
              {description || "American token holders can participate in voting on gameplay updates, seasonal content, and community-driven decisions specific to the U.S. region."}
            </p>
          </div>
        </div>
      )}
