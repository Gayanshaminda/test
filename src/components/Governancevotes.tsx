...
const Governancevotes: React.FC<GovernancevotesProps> = ({ onClick, description }) => {
  ...
  return (
    <div className="pointer-events-auto relative flex w-full">
      {!isLargeScreen && !isExpanded ? (
        <button
          onClick={handleClick}
          onMouseEnter={() => setIsHovered(true)}
          onMouseLeave={() => setIsHovered(false)}
          className={`border-theme-pink-500 bg-card-gradient pointer-events-auto flex h-12 w-full max-w-xs items-center justify-between rounded-[36px] border px-5 py-3 text-white transition-all duration-300 ease-in-out sm:max-w-md md:max-w-lg ${isHovered ? "shadow-theme-glow" : ""} `}
        >
        ...
        >
          ...
          >
            ...
            >
              <span className="text-theme-pink-400">2 : </span>Governance Votes
            </h3>
            {!isLargeScreen && (
              <img
                src={parkdownIcon}
