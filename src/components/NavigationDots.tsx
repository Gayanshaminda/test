...
const NavigationDots: React.FC<NavigationDotsProps> = ({ 
  ...
  return (
    <div className={`flex items-center justify-center gap-1 sm:gap-1.5 md:gap-2 lg:flex-row ${orientation === 'vertical' ? 'flex-col' : 'flex-row'} ${className}`}>      {pages.map((page) => (
        <NavLink
          key={page}
          to={getPageUrl(page)}
          className={() => {
            ...
            return variant === 'dots'
              ? `rounded-full transition-duration-300 cursor-pointer block                 ${orientation === 'vertical' && !isActive ? 'h-[8px] w-[8px] sm:h-[10px] sm:w-[10px] md:h-[12px] md:w-[12px]' : ''}
                 ${orientation === 'vertical' && isActive ? 'h-[25px] w-[8px] sm:h-[30px] sm:w-[10px] md:h-[40px] md:w-[12px]' : ''}
                 ${orientation === 'horizontal' && !isActive ? 'h-[8px] w-[8px] sm:h-[10px] sm:w-[10px] md:h-[12px] md:w-[12px]' : ''}
                 ${orientation === 'horizontal' && isActive ? 'h-[8px] w-[25px] sm:h-[10px] sm:w-[30px] md:h-[12px] md:w-[40px]' : ''}
                 ${isActive 
        ...
        >
          ...
          {variant === 'dots' && <span className="sr-only">Page {page === 2 ? 4 : page === 3 ? 6 : page === 4 ? 8 : page}</span>}
        </NavLink>
      ))}
    </div>
  );
